    ...
    with open(train_file, 'w') as f:
        ...
        for epoch in range(1, epochs + 1):
            ...
            for i, (x, y, yloss_w) in enumerate(train_loader):                
                ...
                loss, l_recon, l_margin = loss_fn(objcaps_len_step, read_x_step, c_step, readout_logits, x, y, yloss_w, args, device)
                train_loss = loss.item()
                train_recon_loss = l_recon.item()
                train_margin_loss = l_margin.item()
                
                running_train_loss += train_loss
                running_train_recon_loss += train_recon_loss
                running_train_margin_loss += train_margin_loss
            ...
            avg_train_recon_loss = running_train_recon_loss / (i + 1)
            avg_train_margin_loss = running_train_margin_loss / (i + 1)
            ...
            for j, (x, y, yloss_w) in enumerate(val_loader):
                ...
                val_margin_loss = l_margin.item()

                running_val_loss += val_loss
                running_val_recon_loss += val_recon_loss
                running_val_margin_loss += val_margin_loss
            ...
            avg_val_recon_loss = running_val_recon_loss / (j + 1)
            avg_val_margin_loss = running_val_margin_loss / (j + 1)
            ...
            row = [
                ...
                epoch, # 'Epoch'
                avg_train_loss, # 'Loss'
                avg_train_recon_loss, #'Recon loss'
                avg_train_margin_loss, # 'Margin loss'
                avg_train_acc, # 'Accuracy'
                avg_val_loss, # 'Val loss'
                avg_val_recon_loss, #'Val recon loss'
                avg_val_margin_loss, # 'Val margin loss'
            ...
            if epoch % 1 == 0:
                print('[Epoch: ' + str(epoch) + '] ' + \
                        '[t_r_l = ' + '{:.4f}'.format(avg_train_recon_loss) + '] ' + \
                        '[t_m_l = ' + '{:.4f}'.format(avg_train_margin_loss) + '] ' + \
                        '[v_r_l = ' + '{:.4f}'.format(avg_val_recon_loss) + '] ' + \
                        '[v_m_l = ' + '{:.4f}'.format(avg_val_margin_loss) + '] ' + \
...
if __name__ == "__main__":
    ...
    # Train on SVRT-1
    current_dir = 'results/RMTS_OCRA_new'
    check_path(current_dir)
    
    train_ds = RMTS_dataset(
        root_dir='data/svrt1_rmts',
        split='train',
        normalization=None,
        squeeze=False, 
    ...
    for i in range(9, RUNS+1): #RUNS+1
        # Insantiate model
        model = OCRA(args)
    ...
    test_df_name = f'{current_dir}/OCRA_test_RMTS.csv'
    test_df.to_csv(test_df_name)

    train_df = pd.concat(train_df)
    train_df_name = f'{current_dir}/OCRA_train_RMTS.csv'
