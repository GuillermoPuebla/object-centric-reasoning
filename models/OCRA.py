...
# OCRA modules and loss functions
...
class OCRA(nn.Module):
    ...
    def __init__(self, args):
        ...
        # if(self.use_backbone):
            # self.bb_cnn = nn.Sequential(
                # nn.Conv2d(in_channels=self.C, out_channels=args.conv1_nfilters, kernel_size=5, padding=2),
                # nn.ReLU(),
                # nn.MaxPool2d(kernel_size=2, stride=2, padding=0),
                # nn.Conv2d(in_channels=args.conv1_nfilters, out_channels=args.conv2_nfilters, kernel_size=3, padding=1),
                # nn.ReLU(),
                # nn.MaxPool2d(kernel_size=2, stride=2, padding=0)
                # )
        ...
        if(self.use_backbone == 'conv_small'):
            ...
            #self.bb_conv3 = nn.Conv2d(self.conv1_nfilters, self.conv2_nfilters, kernel_size=3, stride=1, padding=1)
        ...
        if self.use_read_attn:
            if self.read_size == 18:
                ...
                bb2encoder_size = 288
            elif self.read_size == 36: # Added
                bb2encoder_size = 2592
        else:
            bb2encoder_size = 2048
        ...
        # if self.task == 'svrt_task1':
            # self.hist = nn.LSTMCell(self.num_objectcaps * self.dim_objectcaps, self.lstm_size)   
            #self.readout_0 = nn.Linear(self.lstm_size, 2)
    ...
    def forward(self, x, y=None):
        ...
        # if self.task == 'svrt_task1':
            # h_hist = torch.zeros(batch_size, self.lstm_size).to(device)
            # c_hist = torch.zeros_like(h_enc)  
        ...
        for t in range(self.time_steps):
            ...
            if self.use_read_attn:
                ...
                read_x_step[:,t:t+1,:] = torch.unsqueeze(x.view(x.shape[0], -1), 1).detach()      
            ...
            if(self.use_backbone == "conv_small"):
                ...
                #r_conv3 = F.relu(F.max_pool2d(self.bb_conv3(r_conv2), 2))
            ...
            if self.decoder_encoder_feedback:
                ...
                # if self.task == 'svrt_task1':
                    # h_enc, c_enc = self.encoder(torch.cat([r, h_dec, h_hist], dim=1), (h_enc, c_enc))
                # else:
            ...
            if self.use_capsnet:      
                ...
                if self.mask_objectcaps:
                    ...
                    #objcaps_masked_len = objectcaps_masked.norm(dim=-1)
                else:
                    objectcaps_re = objectcaps.contiguous().view(objectcaps.size(0), -1)
                    
                # print(y_onehot)
                # print(objcaps_len)
                objcaps_len_step[:,t:t+1,:] = torch.unsqueeze(objcaps_len, 1) #+ 0.2*torch.unsqueeze(y_onehot.detach()*objcaps_len, 1) 
            ...
            # if self.task == 'svrt_task1':
                # h_hist, c_hist = self.decoder(objectcaps_re.detach(), (h_hist, c_hist))
            ...
            # if self.task == 'svrt_task1':
                # readout_logits = self.readout_0(h_hist)
...
def loss_fn(objcaps_len_step, read_x_step, c_step, readout_logits, x, y_true, yloss_w, args, device, writer=None, step=None):
    """
    ...
    L_recon = args.lam_recon * nn.MSELoss()(c, x)    # reduction=None
    # L_recon = L_recon.mean(dim=1) * (2-(yloss_w[:,2]/3))
    ...
    if len(y_true.shape) == 3: 
        ...
        y_true_1d = y_true
        # if args.task == 'svrt_task1':
            # y_true_1d = y_true.narrow(1,0,2)
    ...
    L_margin = L_present+L_absent
    #print(L_margin)
    if args.use_capsnet:
        L_margin = (yloss_w*(L_margin))
    #print(L_margin)
    ...
    if args.task == 'multisvhn':
        #print(y_true[1])
        col_11 = torch.zeros((y_true.size(0),5, 1)).to(device)
    ...
    # #     L_present =  y_true * torch.clamp((y_true-m_neg) - y_pred, min=0.) ** 2   # not clamped version
        ...
        # L_margin = L_margin + (L_margin_digits)
        
    # orignial version in dynamic routin between capsules 
...
def cal_accs(y_pred_nar, y_true, readout_logits, args):
    ...
    if args.task=='multisvhn':  # when there are two targets with duplicates allowed
        ...
        # just get topk accuracy
        _, exact_accs = get_topkacc(y_pred_nar[:,-2:], y_true[:,-2:], topk=1)
        #_, exact_accs = get_topkacc(readout_logits, y_true.narrow(1,2,2), topk=1)
        #_, exact_accs = get_topkacc(y_pred_nar, y_true.narrow(1,2,2), topk=1)
        exact_accuracy = exact_accs.cpu().sum().item()
        
        partial_accs = exact_accs
        partial_accuracy = exact_accuracy
        
        #y_pred_hot = readout_logits #
        y_pred_hot = y_pred_nar # torch.sum(y_pred_nar, dim=1)
...
@torch.no_grad()
def evaluate(model, x, y_true, loss_fn, args, device, epoch=None):
    """
    ...
    yloss_w = torch.ones((len(x),args.num_classes)).to(device)
    ...
    # compute loss    
    loss, L_recon, L_margin = loss_fn(objcaps_len_step, read_x_step, c_step, readout_logits, x, y_true, yloss_w, args, device)        
...
def test(model, dataloader, args):
    ...
    for x, y in dataloader:
        ...
        test_L_recon += batch_L_recon
        test_L_margin += batch_L_margin
