...
class ViT(nn.Module):
	def __init__(self, dim, depth, heads, mlp_dim, pool='cls', dim_head=64, dropout=0.0, emb_dropout=0.):
		super().__init__()
...
class scoring_model(nn.Module):
	def __init__(self, in_dim, depth, heads, mlp_dim, num_slots, context_norm):
		...
		self.transformer = ViT(in_dim, depth, heads, mlp_dim, num_slots)
	...
	def forward(self,feat_panels,pos_panels, device):		
		...
		score = self.transformer(sim_matrix_triu_withpos,device).squeeze()
