...
def build_complementary_samples_SOSD(
    base_array,
    same_relation_array,
    diff_relation_array
    ):
    """Build two complementary samples of SOSD"""
    # Coordinates
    left1, top1 = 32, 0
    left2, top2 = 32, 64
    # Get individual images
    base_img = Image.fromarray(base_array)
    match_img = Image.fromarray(same_relation_array)
    nonmatch_img = Image.fromarray(diff_relation_array)
    
    # Second order 'same' sample
    so_same_img = Image.new(base_img.mode, (128, 128), (255, 255, 255))
    so_same_img.paste(base_img, (left1, top1))
    so_same_img.paste(match_img, (left2, top2))
    
    # Second order 'different' sample
    so_diff_img = Image.new(base_img.mode, (128, 128), (255, 255, 255))
    so_diff_img.paste(base_img, (left1, top1))
    so_diff_img.paste(nonmatch_img, (left2, top2))
    
    return so_same_img, so_diff_img
...
def build_complementary_samples(
    ...
    return mn_full, nm_full


# Functions to generate deterministic samples
def make_SOSD_irregular_sample(base_category):
    """Makes two complementary samples of the RMTS task in the 'irregular' condition.
    Args:
        base_category: 0 (diff) or 1 (same).
    """
    while True:
        base_full, _, _ = svrt_1_img(base_category, regular=False, color_a=(0,0,0), sides=None, thickness=1)
        ...
        if bad_sample:
            ...
            break
    while True:
        same_full, _, _ = svrt_1_img(1, regular=False, color_a=(0,0,0), sides=None, thickness=1) # same
        ...
        if bad_sample:
            ...
            break
    while True:
        diff_full, _, _ = svrt_1_img(0, regular=False, color_a=(0,0,0), sides=None, thickness=1) # different
    ...
    # Build samples
    if base_category == 0:
        same_relation_array = diff_full
        diff_relation_array = same_full
    elif base_category == 1:
        same_relation_array = same_full
        diff_relation_array = diff_full
    else:
        raise ValueError('Unrecognized category!')

    return build_complementary_samples_SOSD(
        base_full,
        same_relation_array,
        diff_relation_array
        )

def make_SOSD_regular_sample(base_category):
    """Makes two complementary samples of the RMTS task in the 'regular' condition.
    Args:
        base_category: 0 (diff) or 1 (same).
    """
    while True:
        base_full, _, _ = svrt_1_img(base_category, regular=True, color_a=(0,0,0), sides=None, thickness=1)
        _, _, bad_sample = get_shapes_info(base_full, scrambled_negative=False, draw_b_rects=False)
        # if not bad_sample:
        ...
        if bad_sample:
            ...
            break
    while True:
        same_full, _, _ = svrt_1_img(1, regular=True, color_a=(0,0,0), sides=None, thickness=1) # same
        _, _, bad_sample = get_shapes_info(same_full, scrambled_negative=False, draw_b_rects=False)
        if bad_sample:
            pass
        else:
            break
    while True:
        diff_full, _, _ = svrt_1_img(0, regular=True, color_a=(0,0,0), sides=None, thickness=1) # different
        _, _, bad_sample = get_shapes_info(diff_full, scrambled_negative=False, draw_b_rects=False)
        if bad_sample:
            pass
        else:
...
def make_SOSD_open_sample(base_category):
    ...
    while True:
        ...
        base_full, _, _ = svrt_1_img(base_category, regular=False, color_a=(0,0,0), sides=None, thickness=1, closed=False, radii=(r1, r2))
        _, _, bad_sample = get_shapes_info(base_full, scrambled_negative=False, draw_b_rects=False)
        if bad_sample:
            pass
        else:
    ...
    while True:
        r1 = np.random.randint(10, 14) # np.random.randint(10, 40)
        r2 = np.random.randint(10, 14) #radius_1 #if category==1 else np.random.randint(10, 40)
        same_full, _, _ = svrt_1_img(1, regular=False, color_a=(0,0,0), sides=None, thickness=1, closed=False, radii=(r1, r2)) # same
        _, _, bad_sample = get_shapes_info(same_full, scrambled_negative=False, draw_b_rects=False)
        if bad_sample:
            pass
        else:
    ...
    while True:
        r1 = np.random.randint(10, 14) # np.random.randint(10, 40)
        r2 = np.random.randint(10, 14) #radius_1 #if category==1 else np.random.randint(10, 40)
        diff_full, _, _ = svrt_1_img(0, regular=False, color_a=(0,0,0), sides=None, thickness=1, closed=False, radii=(r1, r2)) # different
        _, _, bad_sample = get_shapes_info(diff_full, scrambled_negative=False, draw_b_rects=False)
        if bad_sample:
            pass
        else:
...
def make_SOSD_wider_sample(base_category):
    """Makes two complementary samples of the RMTS task in the 'wider' condition.
    Args:
        base_category: 0 (diff) or 1 (same).
    """
    while True:
        base_full, _, _ = svrt_1_img(base_category, regular=False, color_a=(0,0,0), sides=None, thickness=2)
        _, _, bad_sample = get_shapes_info(base_full, scrambled_negative=False, draw_b_rects=False)
        if bad_sample:
            pass
        else:
            break
    while True:
        same_full, _, _ = svrt_1_img(1, regular=False, color_a=(0,0,0), sides=None, thickness=2) # same
        _, _, bad_sample = get_shapes_info(same_full, scrambled_negative=False, draw_b_rects=False)
        if bad_sample:
            pass
        else:
            break
    while True:
        diff_full, _, _ = svrt_1_img(0, regular=False, color_a=(0,0,0), sides=None, thickness=2) # different
        _, _, bad_sample = get_shapes_info(diff_full, scrambled_negative=False, draw_b_rects=False)
        if bad_sample:
            pass
        else:
...
def make_SOSD_scrambled_sample(base_category):
...
def make_SOSD_randomcolor_sample(base_category):
    ...
    while True:
        ...
        base_full, _, _ = svrt_1_img(base_category, regular=False, color_a=color, sides=None, thickness=1)
        _, _, bad_sample = get_shapes_info(base_full, scrambled_negative=False, draw_b_rects=False)
        if bad_sample:
            pass
        else:
    ...
    while True:
        color = tuple(np.random.randint(251, size=3))
        color = (int(color[0]), int(color[1]), int(color[2]))
        same_full, _, _ = svrt_1_img(1, regular=False, color_a=color, sides=None, thickness=1) # same
        _, _, bad_sample = get_shapes_info(same_full, scrambled_negative=False, draw_b_rects=False)
        if bad_sample:
            pass
        else:
    ...
    while True:
        color = tuple(np.random.randint(251, size=3))
        color = (int(color[0]), int(color[1]), int(color[2]))
        diff_full, _, _ = svrt_1_img(0, regular=False, color_a=color, sides=None, thickness=1) # different
        _, _, bad_sample = get_shapes_info(diff_full, scrambled_negative=False, draw_b_rects=False)
        if bad_sample:
            pass
        else:
...
def make_SOSD_filled_sample(base_category):
    """Makes two complementary samples of the RMTS task in the 'filled' condition.
    Args:
        base_category: 0 (diff) or 1 (same).
    """
    while True:
        base_full, _, _ = svrt_1_img(base_category, regular=False, color_a=(0,0,0), sides=None, thickness=1, filled=True)
        _, _, bad_sample = get_shapes_info(base_full, scrambled_negative=False, draw_b_rects=False)
        if bad_sample:
            pass
        else:
            break
    while True:
        same_full, _, _ = svrt_1_img(1, regular=False, color_a=(0,0,0), sides=None, thickness=1, filled=True) # same
        _, _, bad_sample = get_shapes_info(same_full, scrambled_negative=False, draw_b_rects=False)
        if bad_sample:
            pass
        else:
            break
    while True:
        diff_full, _, _ = svrt_1_img(0, regular=False, color_a=(0,0,0), sides=None, thickness=1, filled=True) # different
        _, _, bad_sample = get_shapes_info(diff_full, scrambled_negative=False, draw_b_rects=False)
        if bad_sample:
            pass
        else:
...
def make_SOSD_opensquares_sample(base_category):
    """Makes two complementary samples of the RMTS task in the 'open squares' condition.
    Args:
        base_category: 0 (diff) or 1 (same).
    """
    while True:
        base_full, _, _ = make_connected_open_squares(base_category, line_width=1)
        _, _, bad_sample = get_shapes_info(base_full, scrambled_negative=False, draw_b_rects=False)
        if bad_sample:
            pass
        else:
            break
    while True:
        same_full, _, _ = make_connected_open_squares(1, line_width=1) # same
        _, _, bad_sample = get_shapes_info(same_full, scrambled_negative=False, draw_b_rects=False)
        if bad_sample:
            pass
        else:
            break
    while True:
        diff_full, _, _ = make_connected_open_squares(0, line_width=1) # different
        _, _, bad_sample = get_shapes_info(diff_full, scrambled_negative=False, draw_b_rects=False)
        if bad_sample:
            pass
        else:
...
def make_SOSD_arrows_sample(base_category):
    """Makes two complementary samples of the RMTS task in the 'arrows' condition.
    Args:
        base_category: 0 (diff) or 1 (same).
    """
    while True:
        base_full, _, _ = make_arrows_sd(base_category, continuous=True, line_width=1, hard_test=True)
        _, _, bad_sample = get_shapes_info(base_full, scrambled_negative=False, draw_b_rects=False)
        if bad_sample:
            pass
        else:
            break
    while True:
        same_full, _, _ = make_arrows_sd(1, continuous=True, line_width=1, hard_test=True) # same
        _, _, bad_sample = get_shapes_info(same_full, scrambled_negative=False, draw_b_rects=False)
        if bad_sample:
            pass
        else:
            break
    while True:
        diff_full, _, _ = make_arrows_sd(0, continuous=True, line_width=1, hard_test=True) # different
        _, _, bad_sample = get_shapes_info(diff_full, scrambled_negative=False, draw_b_rects=False)
        if bad_sample:
            pass
        else:
...
def make_SOSD_rectangles_sample(base_category):
    """Makes two complementary samples of the RMTS task in the 'rectangles' condition.
    Args:
        base_category: 0 (diff) or 1 (same).
    """
    while True:
        base_full, _, _ = make_rectangles_sd(base_category, line_thickness=1)
        _, _, bad_sample = get_shapes_info(base_full, scrambled_negative=False, draw_b_rects=False)
        if bad_sample:
            pass
        else:
            break
    while True:
        same_full, _, _ = make_rectangles_sd(1, line_thickness=1) # same
        _, _, bad_sample = get_shapes_info(same_full, scrambled_negative=False, draw_b_rects=False)
        if bad_sample:
            pass
        else:
            break
    while True:
        diff_full, _, _ = make_rectangles_sd(0, line_thickness=1) # different
        _, _, bad_sample = get_shapes_info(diff_full, scrambled_negative=False, draw_b_rects=False)
        if bad_sample:
            pass
        else:
...
def make_SOSD_straightlines_sample(base_category):
    """Makes two complementary samples of the RMTS task in the 'straight lines' condition.
    Args:
        base_category: 0 (diff) or 1 (same).
    """
    while True:
        base_full, _, _ = make_straingt_lines_sd(base_category, var_factor='size', line_thickness=1)
        _, _, bad_sample = get_shapes_info(base_full, scrambled_negative=False, draw_b_rects=False)
        if bad_sample:
            pass
        else:
            break
    while True:
        same_full, _, _ = make_straingt_lines_sd(1, var_factor='size', line_thickness=1) # same
        _, _, bad_sample = get_shapes_info(same_full, scrambled_negative=False, draw_b_rects=False)
        if bad_sample:
            pass
        else:
            break
    while True:
        diff_full, _, _ = make_straingt_lines_sd(0, var_factor='size', line_thickness=1) # different
        _, _, bad_sample = get_shapes_info(diff_full, scrambled_negative=False, draw_b_rects=False)
        if bad_sample:
            pass
        else:
...
def make_SOSD_closedsquares_sample(base_category):
    """Makes two complementary samples of the RMTS task in the 'connected squares' condition.
    Args:
        base_category: 0 (diff) or 1 (same).
    """
    while True:
        base_full, _, _ = make_connected_open_squares(base_category, line_width=1, is_closed=True)
        _, _, bad_sample = get_shapes_info(base_full, scrambled_negative=False, draw_b_rects=False)
        if bad_sample:
            pass
        else:
            break
    while True:
        same_full, _, _ = make_connected_open_squares(1, line_width=1, is_closed=True) # same
        _, _, bad_sample = get_shapes_info(same_full, scrambled_negative=False, draw_b_rects=False)
        if bad_sample:
            pass
        else:
            break
    while True:
        diff_full, _, _ = make_connected_open_squares(0, line_width=1, is_closed=True) # different
        _, _, bad_sample = get_shapes_info(diff_full, scrambled_negative=False, draw_b_rects=False)
        if bad_sample:
            pass
        else:
...
def make_SOSD_connectedcircles_sample(base_category):
    """Makes two complementary samples of the RMTS task in the 'connected circles' condition.
    Args:
        base_category: 0 (diff) or 1 (same).
    """
    while True:
        base_full, _, _ = make_connected_circles(base_category, line_thickness=1)
        _, _, bad_sample = get_shapes_info(base_full, scrambled_negative=False, draw_b_rects=False)
        if bad_sample:
            pass
        else:
            break
    while True:
        same_full, _, _ = make_connected_circles(1, line_thickness=1) # same
        _, _, bad_sample = get_shapes_info(same_full, scrambled_negative=False, draw_b_rects=False)
        if bad_sample:
            pass
        else:
            break
    while True:
        diff_full, _, _ = make_connected_circles(0, line_thickness=1) # different
        _, _, bad_sample = get_shapes_info(diff_full, scrambled_negative=False, draw_b_rects=False)
        if bad_sample:
            pass
        else:
...
def make_SOSD_datasets(train_size, val_size, test_size):
    # Gather all ds generators
    datasets = [
        make_SOSD_irregular_sample,
        make_SOSD_regular_sample,
        make_SOSD_open_sample,
        make_SOSD_wider_sample,
        make_SOSD_scrambled_sample,
        make_SOSD_randomcolor_sample,
        make_SOSD_filled_sample,
        make_SOSD_opensquares_sample,
        make_SOSD_arrows_sample,
        make_SOSD_rectangles_sample,
        make_SOSD_straightlines_sample,
        make_SOSD_closedsquares_sample,
        make_SOSD_connectedcircles_sample
        ]
    # Get all ds names
    ds_names = [
        'irregular_sosd',
    ...
    for ds, ds_name in zip(datasets, ds_names):
        ...
        for split, size in zip(splits, sizes):
            ...
            ds_file_header = ['ID', 'top label', 'bottom label', 'label']
            # Generate data
            counter = 0
            ...
            with open(ds_file, 'w') as f:
                ...
                for i in range(size):
                    ...
                    so_same_img, so_diff_img = gen(base_category)
                    
                    # Save second order 'same' sample
                    so_same_img.save(f'{ds_dir}/{counter}.png')
                    top_label = base_category
                    bottom_label = top_label
                    row = [counter, top_label, bottom_label, 1] 
                    writer.writerow(row)
                    counter += 1

                    # Save second order 'different' sample
                    so_diff_img.save(f'{ds_dir}/{counter}.png')
                    top_label = base_category
                    bottom_label = 0 if top_label == 1 else 1
...
if __name__ == '__main__':
    # Parameters
    TRAIN_SIZE = 49_000 # double
    VAL_SIZE = 7_000 # double
    TEST_SIZE = 14_000 # double
