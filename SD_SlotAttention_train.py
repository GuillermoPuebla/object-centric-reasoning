...
def get_ds_names_and_loaders(batch_size, normalization='slotattention'):
    ...
    ds = SD_dataset(
        ...
        split='test',
        normalization='randomcolor_slotattention', 
        squeeze=False, 
        invert=False
        )
    loader = DataLoader(ds, batch_size=batch_size)
...
if __name__ == "__main__":
    ...
    # Train on SVRT-1
    current_dir = 'results/SD_SlotAttention'
    ...
    test_df_name = f'{current_dir}/SlotAttention_test_SD.csv'
    test_df.to_csv(test_df_name)
    
    train_df = pd.concat(train_df)
    train_df_name = f'{current_dir}/SlotAttention_train_SD.csv'
